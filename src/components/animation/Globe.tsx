...
import countries from "./global.json";
...
export function Globe({ globeConfig, data }) {
  ...
  const startAnimation = () => {
    ...
    globeRef.current
      ...
      .arcColor(d => d.color)
      .arcAltitude(d => d.arcAlt * 1)
      .arcStroke(() => [0.32, 0.28, 0.3][Math.round(Math.random() * 2)])
      .arcDashLength(defaultProps.arcLength)
      .arcDashInitialGap(d => d.order * 1)
      .arcDashGap(15)
...
export function World(props) {
  ...
  return (
    <Canvas scene={scene} camera={new PerspectiveCamera(50, aspect, 180, 1800)}>
      ...
      <Globe {...props} />
      <OrbitControls enablePan={false} enableZoom={false} minDistance={cameraZ} maxDistance={cameraZ} autoRotateSpeed={1} autoRotate minPolarAngle={Math.PI / 3.5} maxPolarAngle={Math.PI - Math.PI / 3} />
    </Canvas>
  );
}
